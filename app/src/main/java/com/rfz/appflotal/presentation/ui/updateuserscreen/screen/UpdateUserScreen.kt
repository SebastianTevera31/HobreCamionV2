...
fun UpdateUserScreen(
    ...
    val updateUserUiState by updateUserViewModel.updateUserUiState.collectAsState()
    val context = LocalContext.current
    val updateUserStatus = updateUserViewModel.updateUserStatus
    val wifiStatus by updateUserViewModel.wifiStatus.collectAsState()
...
@Composable
fun UpdateUserScreenContent(
    uiState: UpdateUserUiState,
    onSaveClick: () -> Unit,
    onNavigateUp: () -> Unit,
    updateUserData: (name: String, password: String, email: String, country: Pair<Int, String>?, sector: Pair<Int, String>?) -> Unit,
    updateVehicleData: (vehicleType: String, plates: String, temperatureUnit: UnitProvider, pressureUnit: UnitProvider, odometerUnit: UnitProvider) -> Unit,
    modifier: Modifier = Modifier
) {
    Scaffold(
        topBar = {
        ...
        bottomBar = {
            Button(
                onClick = onSaveClick,
                enabled = uiState.isNewUserData || uiState.isNewVehicleData,
...
fun UpdateUserScreenPreview() {
    HombreCamionTheme {
        UpdateUserScreenContent(
            ...
            onSaveClick = {},
            onNavigateUp = {},
            updateUserData = { _, _, _, _, _ -> },
            updateVehicleData = { _, _, _, _, _ -> }
        )
    }
}